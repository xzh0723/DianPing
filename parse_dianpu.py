...
def parse_data(shop, css_result, num_dict, address_dict, address_list, line_dict, tag_dict):
    """
    解析数据
    """
    data = dict()
    ...
    data['address'] = process_tag(xaddress, css_result, address_dict, address_list)
    return data
...
def process_address(address, css_result, address_dict, address_list):
    """
    处理地址信息
    """
        ...
        if item in css_result:
            ...
            for num in list(address_dict.keys()):
                if int(y) > int(num):
                    ...
                    line = address_dict[num]
                    str = address_list[int(line) - 1]
                    word = str[offset - 1]
...
def dsvg_result(css_link):
    ...
    for index, svg_link in enumerate(svg_links):
        ...
        if index == 1:
            ...
            for li in lis:
                y = re.search('M0 (\d+) H600', li['d'], re.S).group(1)
                id_ = li['id']
                address_dict[y] = id_
            print(address_dict)
            lis = soup.select('textPath')
            ...
            for num in nums:
                ...
                num_dict[y] = num
            print(num_dict)
    return num_dict, address_dict, address_list, line_dict, tag_dict
